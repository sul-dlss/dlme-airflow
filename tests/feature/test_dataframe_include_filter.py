...
def test_filter_walters_mena_include_filters(
    ...
    assert set(source_dataframe.ObjectID).difference(
        set(filtered_dataframe.ObjectID)
    ) == {11, 14, 74}


def test_filter_walters_mena_islamic_art_include_filters(
    mocker, mock_collection_datafile, tmp_path
):
    provider = Provider("walters")
