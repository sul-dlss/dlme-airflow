...
def validate_metadata_folder(**kwargs):
    ...
    if not os.path.exists(metadata_directory):
        return 'validate_metadata.clone_metadata'

    if len(os.listdir(metadata_directory)) == 0:
        return 'validate_metadata.clone_metadata'

    return 'validate_metadata.pull_metadata'

def build_validate_metadata_taskgroup(dag: DAG) -> TaskGroup:
    validate_metadata_taskgroup = TaskGroup(group_id="validate_metadata")

    bash_command_configure = f"git config --global user.email \"aaron.collier@stanford.edu\" && git config --global user.name \"Aaron Collier\""
    configure_git = BashOperator(task_id='configure_git', bash_command=bash_command_configure, task_group=validate_metadata_taskgroup, dag=dag)
    
    """ Validates if the git folder is empty or not """
    validate_git_folder = BranchPythonOperator(task_id='validate_metadata_folder',
                                                python_callable=validate_metadata_folder, task_group=validate_metadata_taskgroup, dag=dag)
    ...
    git_clone = BashOperator(task_id='clone_metadata', bash_command=bash_command_clone, task_group=validate_metadata_taskgroup, dag=dag)
    ...
    git_pull = BashOperator(task_id='pull_metadata', bash_command=bash_command_pull, task_group=validate_metadata_taskgroup, dag=dag)

    """ Dummy operator (DO NOT DELETE, IT WOULD BREAK THE FLOW) """
    finished_pulling = DummyOperator(task_id='finished_pulling', trigger_rule='none_failed', task_group=validate_metadata_taskgroup, dag=dag)
