...
def data_source_post_harvester(**kwargs):
    ...
    source_provider = provider_key(**kwargs) # gets scripts name
    logging.info(f'provider form kwargs: {kwargs.get(PROVIDER, None)}')
    logging.info(f'collection form kwargs: {kwargs.get(COLLECTION, None)}')
    logging.info(f'post_harvest form kwargs: {kwargs.get(POST_HARVEST, None)}')

    # run python script
    data_source_post_harvester(provider=kwargs[PROVIDER], collection=kwargs.get(COLLECTION, None), post_harvest=kwargs.get(POST_HARVEST, None))
    ...
    #         data_source_post_harvester(provider=kwargs[PROVIDER], collection=collection, post_harvest=collection.get(post_harvest))
    # except TypeError:
