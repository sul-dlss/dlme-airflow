...
def main():
    ...
    if args.d:
        ...
        with open(f"{os.getcwd()}/../dlme-transform/traject_configs/{args.d}.rb", 'r') as traject_config:
            ...
            for l in lines:
                if 'column' in l:
                    mapped_columns.append(l.split("column('")[-1].split("')")[0])
            if args.r:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d",
        help="This flag allows you to pass a path to a traject config file in and a sorted list of fields in the provider data that are not in the config.",
    )
    parser.add_argument(
        "-r",
        action='store_true',
        help="This flag is used to reverse the order of search. It returns a list of fields in the traject config that are not column names in the source data.",
    )
